...
@implementation GLSignleLinkedList {
    //头结点
...
- (void) addElement:(id)element atIndex:(int)index {
    //0.先判断index == 0的情况
    if (index == 0) {
        _first = [[GLSignleLinkedListNode alloc] initWithElement:element andNext:_first];
    } else {
        //1.先获取索引为(index - 1)处的节点
        GLSignleLinkedListNode *prev = [self nodeWithIndex:index - 1];
        prev.next = [[GLSignleLinkedListNode alloc] initWithElement:element andNext:prev.next];
    }
    // 节点数量增加
    _size++;
...
- (id) removeAtIndex:(int)index {
    ...
    GLSignleLinkedListNode *nodeToRemove;
...
- (id) setElement:(id)element atIndex:(int)index {
    //1. 获取index处的节点
    GLSignleLinkedListNode *node = [self nodeWithIndex:index];
    //2. 获取index节点的数据
...
/// 获取index处的的节点
/// @param index index
- (GLSignleLinkedListNode *) nodeWithIndex:(int)index {
    [self checkIndex:index];
    GLSignleLinkedListNode *node = _first;

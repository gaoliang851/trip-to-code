...
/// 链表节点类
fileprivate class SignleLinkedListNode <T: Equatable> : CustomStringConvertible {
    /// 要保存的数据
    var element: T
    /// next指针
    var next: SignleLinkedListNode?
    
    init(element: T,next: SignleLinkedListNode?) {
...
public class LinkedList <T:Equatable> :CustomStringConvertible {
    ...
    /// 头结点
    private var first: SignleLinkedListNode<T>?
    ...
    func addElement(element: T,index: Int) {
        /// 这里要对index == 0做判断
        if index == 0 { //向首节点插入
            first = SignleLinkedListNode(element: element, next: first)
        }
        else { //向其他位置插入
            //1. 先获取前节点
            let prev = node(index: index - 1)
            prev.next = SignleLinkedListNode(element: element, next: prev.next)
        }
        size = size + 1
    ...
    /// - Parameter index: index
    /// - Returns: node
    private func node(index: Int) -> SignleLinkedListNode<T> {
        /// 检查索引合法性
        checkRange(index: index)
        /// 遍历链表
        var node = first!;
